...
//				this.timeTaskDao.insertTimeTask(timeTask);
	...
	@Override
	public Integer updateAuditingAgreement(Integer agreementId) throws Exception {
		Agreement agreement = this.agreementDao.selectAgreementById(agreementId);
		List<SystemUser> systemUserList = this.systemUserDao.selectSystemUserListByPermission(4);
		String email = "";
		for(int k = 0; k < systemUserList.size(); k++){
			SystemUser systemUser = systemUserList.get(k);
			email += "," + systemUser.getUserEmail();
		}
		if(!("".equals(email)))	email = email.substring(1);
		String comment = "立合单：" + agreement.getAgreementName() + "审核";
		TimeTask timeTask = new TimeTask();
		timeTask.setOrderId(agreement.getOrderId());
		timeTask.setNoticeColor(0);
		timeTask.setNoticeType(41);
		timeTask.setPublishCode(agreement.getPublishCode());
		timeTask.setTaskTypeName("立合管理表");
		timeTask.setFailedNum(0);
		timeTask.setComment(comment);
		timeTask.setUserEmail(email);
		this.timeTaskDao.insertTimeTask(timeTask);
		return this.agreementDao.updateAuditingAgreement(agreementId);
	}

	@Override
	public Integer updateAcceptAgreement(Integer agreementId) throws Exception {
		Agreement agreement = new Agreement();
		agreement.setAgreementId(agreementId);
		agreement.setCloseState(4);
		agreement.setAgreementState("10C");
		return this.agreementDao.updateAgreement(agreement);
	}

	@Override
	public Integer updateRefuseAgreement(Integer agreementId, String refuseText) throws Exception {
		Agreement agreement = this.agreementDao.selectAgreementById(agreementId);
		Integer createUserId = agreement.getCreateUserId();
		UserQuery userQuery = new UserQuery();
		userQuery.setUserId(createUserId);
		User user = this.userDao.selectByQuery(userQuery);
		String comment = "立合审核被拒绝--拒绝原因：" + refuseText;
		TimeTask timeTask = new TimeTask();
		timeTask.setOrderId(agreement.getOrderId());
		timeTask.setNoticeColor(0);
		timeTask.setNoticeType(42);
		timeTask.setPublishCode(agreement.getPublishCode());
		timeTask.setTaskTypeName("立合管理表");
		timeTask.setFailedNum(0);
		timeTask.setComment(comment);
