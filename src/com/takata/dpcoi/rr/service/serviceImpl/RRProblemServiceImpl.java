...
import com.takata.dpcoi.rr.dao.RRDelayLeaderDao;
import com.takata.dpcoi.rr.dao.RRProblemDao;
...
public class RRProblemServiceImpl implements RRProblemService {
    ...
    public Integer queryMinisterJurisdiction(SystemUser systemUser) throws Exception {
        return this.systemUserDao.selectMinisterJurisdiction(systemUser);
    }
    ...
    public FileUpload addUploadFile(Integer rrProblemId, String fileAttr, MultipartFile file, String path, SystemUser systemUser) throws Exception {
        ...
        if(rrProblemId != null){
            ...
            }else if("pfmea".equals(fileAttr)){
                rrProblem.setPfmea(formatter.format(new Date()));
                rrProblem.setPfmeaFileId(fileUpload.getFileId());
            }else if("cp".equals(fileAttr)){
                rrProblem.setCp(formatter.format(new Date()));
                rrProblem.setCpFileId(fileUpload.getFileId());
    ...
    private List<Calendar> queryHolidayList(String beginDate){
        ...
        if(beginDate == null || "".equals(beginDate)){
            dateList = this.systemHolidayDao.selectSystemHolidayList();
        }else {
            dateList = this.systemHolidayDao.selectSystemHolidayListFromBeginDate(beginDate);
        }
